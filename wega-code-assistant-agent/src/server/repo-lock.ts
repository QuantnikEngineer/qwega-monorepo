...
async function stageAndCommitIfNeeded(localPath: string, gitAuth: GitAuth | null, sessionId?: string) {
  ...
  const authorName = resolveConfigValue(sessionId, "commitAuthorName", "REPO_LOCK_GIT_COMMIT_AUTHOR_NAME", "WEGA Repo Sync Bot");
  const authorEmail = resolveConfigValue(sessionId, "commitAuthorEmail", "REPO_LOCK_GIT_COMMIT_AUTHOR_EMAIL", "wega-repo-sync-bot@local");
  const commitMessage =
