#!/bin/bash
# ---------------------------------------------------------------
# Cloud Run Deployment Script for wega-code-assistant-agent
# ---------------------------------------------------------------
# Prerequisites:
...
SERVICE_NAME="${CLOUD_RUN_SERVICE:-dev-wega-code-assistant-agent}"
IMAGE="${CLOUD_RUN_IMAGE:-us-central1-docker.pkg.dev/${GCP_PROJECT_ID}/wega-repo/wega-code-assistant-agent:latest}"
...
docker build -t wega-code-assistant-agent .

# ---------------------------------------------------------------
# STEP 2: Tag Image for Artifact Registry
# ---------------------------------------------------------------
echo "=== Tagging Image for Artifact Registry ==="
docker tag wega-code-assistant-agent "${IMAGE}"
