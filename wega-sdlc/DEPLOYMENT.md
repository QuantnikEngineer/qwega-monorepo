# WEGA Frontend — Deployment Guide
...
|----------|----------|-------------|
| `GATEWAY_UPSTREAM` | ✅ | API Gateway Cloud Run URL (e.g., `https://wega-api-gateway-*.run.app`) |
...
Requires `wega-api-gateway` running on port 8080.
...
6. **Deployment order matters**: `wega-auth-service` → `wega-api-gateway` → `wega-frontend`. Each depends on the previous. The frontend needs the gateway URL at deploy time.
