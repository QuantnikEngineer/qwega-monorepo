...
React SPA for the WEGA SDLC automation platform. Provides conversational AI interfaces, admin panels, and role-based dashboards.
...
| wega-api-gateway | Running | Must be available at `:8080` |
| wega-auth-service | Running | Gateway depends on it |
...
Ensure `wega-api-gateway` is running on port 8080.
...
See the auth-service repo (`wega-auth-service/TESTING.md § Roles & Capabilities`) for the full RBAC reference.
...
```bash
# Standard build
docker build -t wega-frontend .

# Override base image registry (used by CI/CD pipelines)
docker build --build-arg BASE_REGISTRY=your-registry.example.com -t wega-frontend .

docker run -p 8080:8080 -e GATEWAY_UPSTREAM=http://gateway-host:8080 wega-frontend
```
...
- **wega-auth-service** and **wega-api-gateway** already deployed (this service depends on both)
...
- Deploys to Cloud Run with profile-based naming (`dev-wega-frontend`, `prod-wega-frontend`)
...
```bash
# Health check
curl https://dev-wega-frontend-XXXXX.us-central1.run.app/health
# Expected: healthy

# Login page loads
curl -s https://dev-wega-frontend-XXXXX.us-central1.run.app/login | head -20
