"""Generate a professional Architecture Document for wega-evals in DOCX format."""
...
def generate_architecture_doc():
    ...
    doc.add_paragraph(
        "Wega-Evals is a reusable, profile-driven evaluation framework purpose-built "
    ...
    components = [
        ...
        ["CLI\n(cli.py)", "argparse-based interface with subcommands: list-agents, seed, run, trace, add-item. Entry point: `wega-eval`."],
    ...
    doc.add_paragraph("Entry point: wega-eval (installed via pip)")
    ...
    _add_code_block(doc,
        ...
        "wega-eval --agent cara seed --limit 50\n"
        "\n"
        "# Run full evaluation\n"
        "wega-eval --agent cara run --dataset cara-eval --concurrency 4\n"
        "\n"
        "# Evaluate specific dimensions only\n"
        "wega-eval --agent cara run --dimensions hallucination,toxicity\n"
        "\n"
        "# Evaluate a single trace\n"
        "wega-eval --agent cara trace abc123def456"
    )
    ...
    doc.add_paragraph(
        ...
        "in wega_evals/profiles/ with a decorated class:"
    )
    _add_code_block(doc,
        '@register_profile\n'
    ...
    run = footer.add_run(
        f"Generated by wega-evals \u2022 {datetime.now().strftime('%Y-%m-%d %H:%M')}"
    )
    run.font.size = Pt(8)
    run.font.color.rgb = GREY
    ...
    output_path = os.path.join(
        os.path.dirname(__file__), "wega_evals_architecture.docx"
    )
