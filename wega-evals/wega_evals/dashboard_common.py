"""Shared Streamlit utilities for the Wega Evals dashboards.

Contains:
- WEGA sunset-dark theme constants & global CSS
...
from wega_evals import langfuse_client as lf
...
def bootstrap() -> None:
    ...
    if not st.session_state.get("_wega_bootstrap_done"):
        from dotenv import load_dotenv

        load_dotenv()
        _ensure_ca_bundle()
        st.session_state["_wega_bootstrap_done"] = True
...
<style>
  ...
  .wega-topbar .crumb {{ color:{COLOR_DIM}; }}
  .wega-topbar .crumb b {{ color:{COLOR_TEXT}; }}
  .wega-chip {{
  ...
  .wega-chip.live  {{ color:{COLOR_GREEN};  border-color:{COLOR_GREEN}; }}
  .wega-chip.warn  {{ color:{COLOR_YELLOW}; border-color:{COLOR_YELLOW}; }}
  .wega-chip.alert {{ color:{COLOR_ORANGE}; border-color:{COLOR_ORANGE}; }}
  .wega-chip.model {{ color:{COLOR_PURPLE}; border-color:{COLOR_PURPLE}; }}
  ...
  .wega-card .head {{ display:flex; justify-content:space-between; align-items:center; margin-bottom:8px; }}
  .wega-card .head .title {{ color:{COLOR_DIM}; font-size:12px; letter-spacing:0.5px; }}
  .wega-card .head .title b {{ color:{COLOR_TEXT}; }}
  .wega-card .head .badge {{ color:{COLOR_DIM}; font-size:11px; border:1px solid #2A303C; padding:1px 8px; border-radius:2px; }}
...
def topbar(page: str, score_count: int, trace_count: int, obs_count: int) -> None:
    ...
    html = (
        f'<div class="wega-topbar">'
        f'<div class="crumb">~/<b>wega-evals</b>/<b>{page}</b></div>'
        f"<div>"
        f'<span class="wega-chip">refresh {last_refresh}</span>'
        f'<span class="wega-chip">scores {score_count}</span>'
        f'<span class="wega-chip">traces {trace_count}</span>'
        f'<span class="wega-chip">obs {obs_count}</span>'
        f'<span class="wega-chip model">langfuse {langfuse_host or "n/a"}</span>'
        f'<span class="wega-chip {live_cls}">{live_txt}</span>'
        f"</div>"
        f"</div>"
    )
    st.markdown(html, unsafe_allow_html=True)
...
def sidebar_header() -> None:
    st.markdown(
        f"<div style='color:{COLOR_ORANGE}; font-size:18px; font-weight:700;'>WEGA-EVALS_</div>"
...
def sidebar_footer() -> None:
    html = (
        ...
        f"<span style='color:{COLOR_ORANGE}'>wega</span>"
        f"</div>"
    )
    st.markdown(html, unsafe_allow_html=True)
