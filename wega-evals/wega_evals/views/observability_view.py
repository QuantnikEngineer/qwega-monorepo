"""Observability view (single-page app section).
...
def render(
    ...
    """Render the Observability page body."""
    ...
    error_count = (
        int((obs_df["level"].isin(["ERROR", "WARNING"])).sum()) if not obs_df.empty else 0
    )
    ...
    with tab_cost:
        ...
        card_open(
            "TOP 20 EXPENSIVE TRACES", "/expensive-traces", accent=COLOR_RED, badge="$ desc"
        )
    ...
    with tab_perf:
        ...
        with c2:
            ...
            if not traces_df.empty:
                ...
                st.markdown(
                    f"<div style='color:{COLOR_DIM}; font-size:11px; margin-top:6px;'>"
                    f"p95 latency</div>",
                    unsafe_allow_html=True,
                )
        ...
        with c4:
            ...
            if not obs_df.empty and "level" in obs_df:
                levels = obs_df[obs_df["level"] != ""].groupby("level").size()
                if levels.empty:
    ...
    with tab_sessions:
        ...
        with c2:
            card_open(
                "TRACES PER SESSION", "/traces-per-session", accent=COLOR_PURPLE, badge="dist"
            )
            if not traces_df.empty and "session_id" in traces_df:
                counts = traces_df[traces_df["session_id"] != ""].groupby("session_id").size()
                if counts.empty:
