"""Wega Evals - Evaluation dashboard.
...
page_setup("Wega Evals - Evaluation", "📊")
...
with st.sidebar:
    sidebar_header()
    st.markdown(
        f"<div style='color:{COLOR_DIM}; font-size:11px;'>page · <b style='color:{COLOR_TEXT}'>evaluation</b></div>",
    ...
    threshold = st.slider(
        "pass threshold", 0.0, 1.0, PASS_THRESHOLD, 0.05, label_visibility="collapsed"
    )
...
with st.sidebar:
    ...
    selected_agents = st.multiselect(
        "agents", agents_all, default=agents_all, label_visibility="collapsed"
    )
    ...
    selected_dims = st.multiselect(
        "dimensions", dims_all, default=dims_all, label_visibility="collapsed"
    )
    sidebar_footer()
...
if not scores_df.empty:
    mask = scores_df["agent"].isin(selected_agents) & scores_df["dimension"].isin(selected_dims)
    mask &= (scores_df["created_at"] >= start_ts) & (scores_df["created_at"] < end_ts)
    ...
    if filtered_scores.empty:
        ...
        with c1:
            ...
            if not overall_scores.empty:
                ...
                agent_avg = (
                    dim_scores.groupby("agent")["score"].mean().sort_values(ascending=False)
                )
                html_bar_chart(agent_avg, threshold=threshold, fmt="{:.2f}", max_value=1.0)
            card_close()

        with c2:
            card_open("PASS / FAIL", "/pass-fail", accent=COLOR_GREEN, badge=f"thr {threshold:.2f}")
            if not dim_scores.empty:
                pf = dim_scores.copy()
                pf["result"] = pf["score"].apply(lambda x: "Pass" if x >= threshold else "Fail")
            ...
            if not dim_scores.empty:
                ...
                def color_score(val):
                    if pd.isna(val):
                        return ""
                    if val >= threshold:
                        return f"background-color: rgba(186,230,126,0.25); color: {COLOR_GREEN};"
...
with tab_trends:
    ...
    if dim_scores.empty:
        ...
        for agent in sorted(dim_scores["agent"].unique()):
            ...
            # render one sparkline per dimension + mean column
            spark_rows = []
            ...
            for dim_name, series, mean_v in spark_rows:
                ...
                line = (
                    ...
                    f'<div style="width:160px; color:{COLOR_TEXT};">{dim_name}</div>'
                )
                st.markdown(line, unsafe_allow_html=True)
                html_sparkline(series, color=color, threshold=threshold, width=280, height=24)
                st.markdown(
                    f"<div style='color:{color}; font-size:12px; margin-top:-22px; padding-left:460px;'>"
                    f"mean {mean_v:.2f}</div>",
                    unsafe_allow_html=True,
                )
    card_close()
...
with tab_quality:
    if filtered_scores.empty or traces_df.empty:
        ...
        if not overall_scores.empty:
            tscore = (
                overall_scores.groupby("trace_id")["score"].mean().rename("score").reset_index()
            )
        else:
            tscore = (
                dim_scores.groupby("trace_id")["score"].mean().rename("score").reset_index()
            )
        ...
        with c1:
            card_open(
                "SCORE per $ (HIGHER BETTER)", "/score-per-cost", accent=COLOR_ORANGE, badge="bar"
            )
            if not joined.empty:
...
with tab_drill:
    ...
    if not filtered_scores.empty:
        fails = filtered_scores[filtered_scores["score"] < threshold].sort_values("score").head(50)
        if fails.empty:
...
with tab_raw:
    ...
    if not filtered_scores.empty:
        ...
        st.download_button(
            "⬇  download scores csv",
            data=csv,
            file_name=f"wega_scores_{datetime.now():%Y%m%d_%H%M%S}.csv",
            mime="text/csv",
        )
    card_close()
