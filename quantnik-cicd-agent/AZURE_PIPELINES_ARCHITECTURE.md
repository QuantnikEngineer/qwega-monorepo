# QUANTNIK Azure Pipeline Generation Architecture And Workflow

This document describes only the QUANTNIK-side architecture for generating `azure-pipelines.yml`.

It does not describe Azure execution, Azure runtime targets, or downstream deployment topology. The focus is only:

1. how the user enters Azure pipeline requirements in QUANTNIK
2. how QUANTNIK turns those inputs into a structured `ci_pipeline_request`
...
Editable draw.io source: [QUANTNIK_AZURE_PIPELINE_GENERATION.drawio](QUANTNIK_AZURE_PIPELINE_GENERATION.drawio)

## QUANTNIK Generation Architecture

```mermaid
flowchart LR
    Engineer[Developer / Platform Engineer]

    subgraph QUANTNIKUI[QUANTNIK UI]
    ...
    subgraph ORCH[QUANTNIK Orchestration Layer]
        SDLC[SDLC Orchestrator]
        DEPLOY[Deployment Orchestrator]
    end

    subgraph CIAGENT[QUANTNIK CI Agent]
...
sequenceDiagram
    ...
    participant UI as QUANTNIK UI / Preview
...
1. `azure-pipelines.yml` is the primary output artifact QUANTNIK produces for Azure DevOps.
...
3. The QUANTNIK generation boundary ends when the CI Agent returns the YAML artifact and metadata.
4. Azure execution concerns are intentionally out of scope for this document.
5. This document should be used when discussing how QUANTNIK builds the Azure pipeline, not how Azure later runs it.
