...
class LlmPipelineRenderer:
    ...
    def _get_client(self):
        ...
        try:
            ...
            raise PipelineRenderError(
                ['LLM render mode was requested, but the google-genai package is not installed in Quantnik CI Agent.'],
                status_code=503,
            ) from error
        ...
        raise PipelineRenderError(
            ['LLM render mode was requested, but neither GOOGLE_CLOUD_PROJECT nor GOOGLE_API_KEY is configured for Quantnik CI Agent.'],
            status_code=503,
        )
