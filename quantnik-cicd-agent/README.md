...
└───────────────────────────────────────┬─────────────────────────────────────┘
                                        ...
                                        ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                      QUANTNIK CI/CD AGENT (This Project)                        │
│                               Port 8092                                     │
│                                                                             │
│  ┌────────────────────────────────────────────────────────────────────┐    │
...
```json
{
    ...
    "render_mode": "hybrid",
    "repository": {
        "url": "https://github.com/org/quantnik-payment-service.git",
        "branch": "main"
    },
    "target": {
        ...
        "azureDevops": {
            "containerApps": {
                ...
                "resourceGroup": "quantnik-rg",
                "location": "eastus",
                "containerAppName": "quantnik-payment-service",
                "containerAppEnvPrefix": "quantnik-aca",
                "acrName": "quantnikacr",
                "acrLoginServer": "quantnikacr.azurecr.io",
    ...
    "build": {
        ...
        "image_repository": "quantnikacr.azurecr.io/quantnik-payment-service",
...
```json
{
    ...
    "pipelineName": "quantnik-payment-service",
    "platform": "azure-devops",
    "artifact": {
...
```bash
docker build -t quantnik-ci-agent .
docker run -p 8092:8092 --env-file .env quantnik-ci-agent
```
...
variables:
  ...
  acrName: 'quantnikacr'
  acrLoginServer: 'quantnikacr.azurecr.io'
  imageRepository: 'quantnik-payment-service'
...
- [QUANTNIK Deployment Orchestrator](../quantnik-deployment-orchestrator) - Parent orchestrator
- [QUANTNIK SDLC Frontend](../quantnik-sdlc) - Pipeline configuration UI
