...
Updates:
- WEGA10 orchestrator (wegaVersion 1.0 repos)
- WEGA30 orchestrator (wegaVersion 3.0 repos)
...
def repo_name_to_variable_name(repo_name: str) -> str:
    ...
    Examples:
        WEGA-BRD -> branchWEGABRD
        Wega-Orchestrator-Agent -> branchWegaOrchestratorAgent
        WEGA-testcases-to-testdata-Agent -> branchWegaTestcasesToTestdataAgent
    """
...
def build_entries_by_version_and_target(repos: list[dict]) -> dict:
    """
    Build matrix entries organized by wegaVersion and deployTarget.
    ...
    Returns:
        {
            ...
            "3.0": { ... },
            "repo_names": {"1.0": [...], "3.0": [...]},
            "all_repos": [{"repoName": ..., "branch": ..., "wegaVersion": ..., "variableName": ...}, ...]
        }
    """
    result = {
        "1.0": {
    ...
    for repo in repos:
        ...
        wega_version = to_string(repo.get("wegaVersion") or "1.0")
        if wega_version not in ["1.0", "3.0"]:
            wega_version = "1.0"
        ...
        result["repo_names"][wega_version].append(repo_name)
        result["all_repos"].append({
            "repoName": repo_name,
            "branch": branch,
            "wegaVersion": wega_version,
        ...
        # WEGA 3.0 repos go to CloudRun, ACA, and ECS (same repos, different deploy targets)
        # WEGA 1.0 repos use deployTarget to route to specific platforms
        if wega_version == "3.0":
            # All WEGA 3.0 repos go to CloudRun, ACA, and ECS
            result[wega_version]["cloudrun"]["build"].append(cloudrun_build_entry)
            result[wega_version]["cloudrun"]["deploy"].append(cloudrun_deploy_entry)
            result[wega_version]["aca"]["build"].append(aca_build_entry)
            result[wega_version]["aca"]["deploy"].append(aca_deploy_entry)
            result[wega_version]["ecs"]["build"].append(ecs_build_entry)
            result[wega_version]["ecs"]["deploy"].append(ecs_deploy_entry)
        else:
            # WEGA 1.0 repos - route based on deployTarget
...
def update_orchestrator(orchestrator_yaml: dict, repo_names: list[str], all_repos: list[dict], wega_version: str):
    ...
    version_repos = [r for r in all_repos if r["wegaVersion"] == wega_version]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--wega10-orchestrator", default=".harness/pipeline-wega10-orchestrator.yaml",
                        help="Path to WEGA 1.0 orchestrator pipeline")
    parser.add_argument("--wega30-orchestrator", default=".harness/pipeline-wega30-orchestrator.yaml",
                        help="Path to WEGA 3.0 orchestrator pipeline")
...
def main() -> int:
    ...
    print(f"Found {len(entries['repo_names']['1.0'])} WEGA 1.0 repos, {len(entries['repo_names']['3.0'])} WEGA 3.0 repos")
    print(f"Total branch variables to sync: {len(entries['all_repos'])}")

    # Update WEGA 1.0 orchestrator
    wega10_path = Path(args.wega10_orchestrator)
    if wega10_path.exists() and entries["repo_names"]["1.0"]:
        wega10_yaml = load_yaml(wega10_path, yaml_module)
        update_orchestrator(wega10_yaml, entries["repo_names"]["1.0"], entries["all_repos"], "1.0")
        if not args.dry_run:
            save_yaml(wega10_path, wega10_yaml, yaml_module)
        print(f"WEGA10 orchestrator: {len(entries['repo_names']['1.0'])} repos + branch variables")

    # Update WEGA 3.0 orchestrator
    wega30_path = Path(args.wega30_orchestrator)
    if wega30_path.exists() and entries["repo_names"]["3.0"]:
        wega30_yaml = load_yaml(wega30_path, yaml_module)
        update_orchestrator(wega30_yaml, entries["repo_names"]["3.0"], entries["all_repos"], "3.0")
        if not args.dry_run:
            save_yaml(wega30_path, wega30_yaml, yaml_module)
        print(f"WEGA30 orchestrator: {len(entries['repo_names']['3.0'])} repos + branch variables")
