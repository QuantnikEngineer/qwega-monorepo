...
def get_repos_by_version(catalog_yaml: dict) -> dict[str, list[str]]:
    """Get repo names organized by wegaVersion."""
    ...
    for repo in repos:
        ...
        version = str(repo.get("wegaVersion") or "1.0")
...
def main() -> int:
    ...
    if wega10_path.exists() and repos_by_version["1.0"]:
        print(f"\nProcessing WEGA 1.0 orchestrator ({len(repos_by_version['1.0'])} repos)...")
        ...
        if args.dry_run:
            ...
            shutil.copy(wega10_path, tmp_path)
            ...
            updated, notes, changed = update_orchestrator_file(
                wega10_path, repos_by_version["1.0"], origin_url, git_username, git_password
            )
            if changed:
                modified_files.append(str(wega10_path))
        
        for note in notes:
            print(note)
        print(f"WEGA10: {updated} branch variables updated")
        total_updated += updated

    # Update WEGA 3.0 orchestrator
    wega30_path = Path(args.wega30_orchestrator)
    if wega30_path.exists() and repos_by_version["3.0"]:
        print(f"\nProcessing WEGA 3.0 orchestrator ({len(repos_by_version['3.0'])} repos)...")
        ...
        if args.dry_run:
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as tmp:
                tmp_path = Path(tmp.name)
            shutil.copy(wega30_path, tmp_path)
            updated, notes, _ = update_orchestrator_file(
            ...
            updated, notes, changed = update_orchestrator_file(
                wega30_path, repos_by_version["3.0"], origin_url, git_username, git_password
            )
            if changed:
                modified_files.append(str(wega30_path))
        
        for note in notes:
            print(note)
        print(f"WEGA30: {updated} branch variables updated")
        total_updated += updated
