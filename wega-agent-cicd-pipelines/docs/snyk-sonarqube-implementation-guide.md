...
This document outlines comprehensive requirements and implementation steps for all security scanning tools in the Wega Agent CICD Pipeline:
...
- name: aquaCspUrl
  type: String
  value: <+input>.default(https://aqua-csp.wega-prod.svc.cluster.local:8443)
...
- Project Key: `wega-<repo-name>` (e.g., `wega-development-agent`)
- Project Name: `Wega - Development Agent`
...
- step:
    ...
    spec:
      ...
      command: |
        ...
        sonar.projectKey=wega-<+matrix.repo.repoName>
        sonar.projectName=Wega - <+matrix.repo.repoName>
        sonar.sources=.
...
- step:
    ...
    spec:
      ...
      command: |
        ...
        PROJECT_KEY="wega-<+matrix.repo.repoName>"
        TOKEN="<+secrets.getValue("sonarcloudToken")>"
...
- Format: Use consistent naming: `wega-<repo-name>`
