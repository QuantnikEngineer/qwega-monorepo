...
## Overview
This document summarizes the key features of the current WEGA BuildAI pipeline design.
...
- **Cloud Run (GCP)**: Primary deployment target for WEGA 3.0 repos
- **Azure Container Apps (ACA)**: Secondary deployment target for WEGA 3.0 repos
- **EKS (AWS)**: Kubernetes deployment for WEGA 1.0 repos
- **GKE (GCP)**: Alternative Kubernetes deployment
- **AKS (Azure)**: Azure Kubernetes Service deployment
- WEGA 3.0 repos automatically sync to both CloudRun and ACA workers
...
- Service naming uses environment prefix: `<env>-<service-name>` (e.g., `dev-wega-brd-backend`)
