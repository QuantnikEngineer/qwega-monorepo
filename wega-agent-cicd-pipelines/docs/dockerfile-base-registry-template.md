...
All WEGA repositories should update their Dockerfiles to use the `BASE_REGISTRY` build argument. This allows the same Dockerfile to work across CloudRun (GCP), ACA (Azure), and ECS (AWS) deployments.

## Base Image Registries

| Platform | Registry URL |
|----------|--------------|
| **CloudRun (GCP)** | `us-central1-docker.pkg.dev/digital-rig-poc/wega-build-baseimages` |
| **ACA (Azure)** | `wegadev.azurecr.io/base-images` |
