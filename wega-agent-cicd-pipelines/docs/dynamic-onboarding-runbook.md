...
- repoName: your-new-repo
  branch: develop
  wegaVersion: "3.0"          # "1.0" for EKS, "3.0" for CloudRun/ACA
...
3. This will update all worker pipelines:
   ...
   - Orchestrator pipelines (WEGA 1.0 and 3.0)
...
### Step 1: Verify in WEGA 3.0 Orchestrator (Primary)

1. Open `WEGA30Orchestrator` pipeline (or `WEGA10Orchestrator` for 1.0 repos)
...
1. Open the appropriate worker pipeline:
   - `RepoWorkerBuildDeployCloudRun` (for WEGA 3.0)
   - `RepoWorkerBuildDeployACA` (for WEGA 3.0)
   - `RepoWorkerBuildDeployEKS` (for WEGA 1.0)
...
```
Orchestrator (WEGA 3.0)          Worker Pipeline (CloudRun/ACA)
        │                                    │
        ▼                                    ▼
┌─────────────────┐              ┌─────────────────────────┐
