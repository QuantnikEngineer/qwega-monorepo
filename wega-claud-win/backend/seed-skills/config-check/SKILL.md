...
If `wega.json` is absent, halt: "No wega.json — can't tell which Jira/Confluence to validate. Open the project in wega2 first so the sidecar gets written."
...
- **Local path missing OR not a git repo, but remote_url present** → call `POST /api/repos/<projectId>/<repoId>/clone` and mark the row `fixed` on 2xx. The wega2 API uses its own credentials cache, so clone will succeed wherever the user has Git Credential Manager configured for the remote.
...
- If the error message is "tool not available" / "unknown tool", record `fail` with detail "Atlassian Jira MCP is not loaded in this session. Re-run wega2 with the Atlassian MCP env vars set: `MCP_ATLASSIAN_EMAIL`, `MCP_ATLASSIAN_TOKEN`, `MCP_ATLASSIAN_SITE_NAME` (see backend/.env)." — there's nothing to auto-fix from inside the skill.
- If the error is HTTP 401 / 403, record `fail` with detail "Jira auth rejected — the API token in `backend/.env` is invalid or revoked. Generate a new one at id.atlassian.com → Security → API tokens, update `MCP_ATLASSIAN_TOKEN`, and restart wega2."
...
Record:
- HTTP 404 → `fail` ("Jira project `<key>` not found on this site. Either the project was deleted or wega.json points at the wrong key.")
...
- 200 but no space matches → `fail` ("Confluence space `<key>` not found in `<site>`. Either the key is wrong in wega.json or the user's API token doesn't have access to that space.")
...
If `wega.json` has `confluenceSpaceId: null` but step 3.1 just resolved a real ID, that's a real auto-fix. POST it back to wega2:
```
...
## Step 4 — Wega2 service self-check (cheap)

`Bash` `curl -sf http://localhost:6060/api/health` with `timeout: 5000`. 200 → `pass`. Anything else → `fail` (the wega2 backend itself is down — most user-facing actions in the UI will be broken too).
...
Print this exact shape (markdown — wega2 renders it cleanly):
...
📦 Project
  Name:   <project.name>  (wega2 id <project.id>)
  Path:   <project.path>

🔗 Git remotes
  <per-repo line — e.g. ✅ wega-claude-mobile · https://git.harness.io/…  (cloned, ls-remote 200ms)>
...
- **Hard timeout per network call** — `15s` for `git ls-remote`, `10s` for MCP calls, `5s` for the local wega2 health probe. A flaky remote shouldn't hang the whole skill.
...
- **Confluence space key from `wega.json` only.** Never call `getConfluenceSpaces` with no filter to find one — that's the bug this skill is designed to catch.
