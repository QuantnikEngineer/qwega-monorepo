...
const doc = new PDFDocument({
  ...
  info: {
    ...
    Author:   'wega2 / sdlc-tokenomics',
    Subject:  'Phase-by-phase LLM cost mapping',
    Producer: 'wega2 sdlc-tokenomics skill',
  },
});
...
const caveats = (data.caveats && data.caveats.length) ? data.caveats : [
  ...
  'wega2\'s agent runtime is Claude-only today. Non-Claude picks can be costed but cannot yet be executed by the wega2 agent runtime.',
];
...
for (let i = range.start; i < range.start + range.count; i++) {
  ...
  doc.text(`Generated by wega2 · sdlc-tokenomics · ${data.generated_at || new Date().toISOString().slice(0, 10)}`,
