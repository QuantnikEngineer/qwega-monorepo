# Quantnik Auth Service

Authentication and authorization service for the QUANTNIK SDLC automation platform.
...
& $PSQL -U postgres -h localhost -c "CREATE DATABASE quantnik_auth;"
```
...
# DATABASE_URL=postgresql+asyncpg://postgres:YOUR_PASSWORD@localhost:5432/quantnik_auth
...
```bash
# Create a superadmin (from quantnik-auth-service/)
...
```bash
# Standard build
docker build -t quantnik-auth-service .

# Override base image registry (used by CI/CD pipelines)
docker build --build-arg BASE_REGISTRY=your-registry.example.com -t quantnik-auth-service .

docker run -p 8090:8080 --env-file .env quantnik-auth-service
```
...
- Deploys to Cloud Run with profile-based naming (`dev-quantnik-auth-service`, `prod-quantnik-auth-service`)
...
|----------|----------|---------|
| `DATABASE_URL` | Yes | `postgresql+asyncpg://user:pass@host:5432/quantnik_auth` |
...
| `JWT_ISSUER` | No | `quantnik-auth` (default) |
| `JWT_AUDIENCE` | No | `quantnik-api` (default) |
...
| `FRONTEND_URL` | Yes | `https://dev-quantnik-frontend-XXXXX.us-central1.run.app` |
...
> ```bash
> gcloud run jobs update <profile>-quantnik-auth-service-migrate \
>   --region us-central1 --image <same-image-as-service>
> gcloud run jobs execute <profile>-quantnik-auth-service-migrate --wait
> ```
>
...
gcloud run jobs create dev-quantnik-auth-service-migrate \
  --image gcr.io/digital-rig-poc/dev-quantnik-auth-service \
  --region us-central1 \
  --set-env-vars DATABASE_URL=postgresql+asyncpg://user:pass@host:5432/quantnik_auth \
  --command alembic,upgrade,head
gcloud run jobs execute dev-quantnik-auth-service-migrate --region us-central1

# Option B: Run locally (dev — simpler)
export DATABASE_URL=postgresql+asyncpg://user:pass@host:5432/quantnik_auth
alembic upgrade head
```
...
gcloud secrets create dev-quantnik-auth-jwt-private-key --data-file=keys/private.pem
gcloud secrets create dev-quantnik-auth-jwt-public-key --data-file=keys/public.pem

# Database URL
echo -n "postgresql+asyncpg://user:pass@host:5432/quantnik_auth" | \
  gcloud secrets create dev-quantnik-auth-database-url --data-file=-

# Internal API key (shared with api-gateway)
echo -n "$(openssl rand -hex 32)" | \
  gcloud secrets create dev-quantnik-internal-api-key --data-file=-
```
...
for SECRET in dev-quantnik-auth-jwt-private-key dev-quantnik-auth-jwt-public-key dev-quantnik-auth-database-url dev-quantnik-internal-api-key; do
...
```bash
# Health check
curl https://dev-quantnik-auth-service-XXXXX.us-central1.run.app/health
# Expected: {"status": "healthy", ...}

# JWKS endpoint
curl https://dev-quantnik-auth-service-XXXXX.us-central1.run.app/.well-known/jwks.json
...
```
1. quantnik-auth-service  ← YOU ARE HERE
2. quantnik-api-gateway   (needs AUTH_SERVICE_URL pointing to auth)
3. quantnik-frontend      (needs GATEWAY_UPSTREAM pointing to gateway)
```
...
### "database 'quantnik_auth' does not exist"
