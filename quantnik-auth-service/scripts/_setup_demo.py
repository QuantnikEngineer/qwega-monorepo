"""
E2E demo setup — follows the correct QUANTNIK onboarding hierarchy:
...
sa_token, sa_user = login("aniket.ashtikar@wipro.com", "QuantnikAdmin1!@#")
...
PROJECT_ID = create_project(pm_token, "QUANTNIK Platform", "quantnik-platform",
...
r = requests.put(
    ...
    json={
        "config": {
            "url": "https://quantnikbuildiq.atlassian.net",
            "projectKey": "QUANTNIKAIDEMO",
...
PLACEHOLDER_TOOLS = {
    "sonarqube": {"config": {"url": "https://sonarqube.quantnik-infra.com"}, "secrets": {"patToken": ""}},
    "snyk": {"config": {"orgId": "quantnik-org"}, "secrets": {"patToken": ""}},
    "trivy": {"config": {"serverUrl": "https://trivy.quantnik-infra.com"}, "secrets": {}},
    "github": {"config": {"url": "https://github.com/quantnik-platform"}, "secrets": {"patToken": ""}},
    "sharepoint": {"config": {"url": "https://quantnik.sharepoint.com"}, "secrets": {"patToken": ""}},
}
...
print(f"""
  Auth Service: {BASE}
  Project:      QUANTNIK Platform ({PROJECT_ID})

  Users:
    SuperAdmin  aniket.ashtikar@wipro.com  QuantnikAdmin1!@#
