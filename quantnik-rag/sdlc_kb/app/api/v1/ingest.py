...
async def _ingest_website(
    ...
    for doc, _, normalized in created:
        background_tasks.add_task(_run_website_pipeline, str(doc.id), normalized)
...
async def _run_website_pipeline(doc_id: str, normalized: dict):
...
async def _ingest_confluence(
    ...
    token = body.token or settings.CONFLUENCE_TOKEN
    ...
    background_tasks.add_task(_run_confluence_pipeline, str(doc.id), normalized)
...
async def _run_confluence_pipeline(doc_id: str, normalized: dict):
    async with AsyncSessionLocal() as db, AsyncSessionLocalNonCritical() as db_nc:
        pipeline = IngestionPipeline(db, db_nc)
        try:
            await pipeline.run_from_text(doc_id, normalized)
        except Exception as exc:
            logger.error("confluence_pipeline_failed", doc_id=doc_id, error=str(exc))
...
async def _ingest_sharepoint(
    ...
    token = body.token or settings.SHAREPOINT_TOKEN or None
    ...
    for doc, normalized in created:
        background_tasks.add_task(_run_sharepoint_pipeline, str(doc.id), normalized)
...
async def _run_sharepoint_pipeline(doc_id: str, normalized: dict):
    async with AsyncSessionLocal() as db, AsyncSessionLocalNonCritical() as db_nc:
        pipeline = IngestionPipeline(db, db_nc)
        try:
            await pipeline.run_from_text(doc_id, normalized)
        except Exception as exc:
            logger.error("sharepoint_pipeline_failed", doc_id=doc_id, error=str(exc))
...
async def _ingest_repo(
    ...
    token = body.token
    if not token:
        token = settings.HARNESS_TOKEN if platform == "harness" else settings.GITHUB_TOKEN
        token = token or None
    ...
    for doc, normalized in created:
        background_tasks.add_task(_run_repo_pipeline, str(doc.id), normalized)
...
async def _run_repo_pipeline(doc_id: str, normalized: dict):
    async with AsyncSessionLocal() as db, AsyncSessionLocalNonCritical() as db_nc:
        pipeline = IngestionPipeline(db, db_nc)
        try:
            await pipeline.run_from_text(doc_id, normalized)
        except Exception as exc:
            logger.error("repo_pipeline_failed", doc_id=doc_id, error=str(exc))
...
async def _fetch_url_content(url: str, title: str, agent_name: str, artifact_type: str) -> dict:
    if is_confluence_url(url):
        connector = ConfluenceConnector(
            email=settings.CONFLUENCE_EMAIL, token=settings.CONFLUENCE_TOKEN,
        )
    else:
