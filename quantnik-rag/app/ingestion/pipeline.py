...
# Knowledge graph extraction is best-effort — failures never block ingestion
_KG_ENABLED = True
...
class IngestionPipeline:
    ...
    ``db``    → critical Postgres DB (sdlc_kb)
    ``db_nc`` → non-critical Postgres DB (sdlc_kb_non_critical)
    ...
    async def run_from_text(self, doc_id: str, normalized: dict):
        """Entry point for websites, repositories, etc. — skips file parsing."""
        doc = await self._get_doc(doc_id)
    ...
    async def run_agent_output(self, doc_id: str, normalized: dict, sdlc_phase: str):
        """Agent outputs bypass classification — always critical."""
        doc = await self._get_doc(doc_id)
    ...
    async def _index_agent_output(
        ...
        if prev_doc is None or prev_criticality is None:
            ...
            old_vectors = await self.store.retrieve_vectors(unchanged_old_point_ids, prev_criticality)
            ...
            for new_chunk, vector in chunks_reuse_vector:
                all_embedded.append(
                    EmbeddedChunk(
                        ...
                        project_name = new_chunk.project_name,
                        metadata     = new_chunk.metadata,
                        vector       = vector,
                    )
                )

        count = await self.store.upsert(all_embedded, criticality="critical")

        if prev_doc is not None and prev_criticality is not None:
            await self.store.delete_by_document(prev_doc_id, prev_criticality)
    ...
    async def run_feedback(self, doc_id: str, content: str, sdlc_phase: str, artifact_type: str):
        ...
        try:
            ...
            count    = await self.store.upsert(embedded, criticality="critical")
    ...
    async def _classify_and_index(self, doc, normalized: dict, cleanup_path: Path | None):
        ...
        embedded = await self.embedder.embed_chunks(chunks)
        count    = await self.store.upsert(embedded, criticality=criticality)

        if criticality == "non_critical":
            await self._write_non_critical(doc, embedded, count)
            # Remove from critical DB — it was staged there by the upload API
        ...
        # Knowledge graph extraction (best-effort, for critical docs only)
        if _KG_ENABLED and criticality == "critical":
            await self._extract_knowledge_graph(doc_id, normalized["text"], doc.project_name, final["sdlc_phase"])
    ...
    async def _classify_and_index_diff(
        ...
        * The previous DB document + chunk records are removed.
        ...
        count = await self.store.upsert(all_embedded, criticality=new_criticality)
    ...
    async def _extract_knowledge_graph(
        ...
        """Best-effort knowledge graph extraction from document text."""
        try:
            from app.memory.knowledge_graph import KnowledgeGraphExtractor
            extractor = KnowledgeGraphExtractor()
            # Extract from first 4000 chars to limit LLM cost
            ...
            logger.info("pipeline_kg_done", doc_id=doc_id, **result)
        except Exception as e:
            logger.warning("pipeline_kg_failed", doc_id=doc_id, error=str(e))
