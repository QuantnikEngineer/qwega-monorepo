...
from app.api.deps import get_db, get_qdrant_store
...
async def list_documents(
    ...
    query = select(Document).where(Document.project_name == project_name)
    if filename:
        term = f"%{filename}%"
        query = query.where(
            or_(
...
async def delete_document(doc_id: UUID, db: AsyncSession = Depends(get_db)):
    ...
    if not doc:
        raise HTTPException(404, "Document not found")
    await get_qdrant_store().delete_by_document(str(doc_id))
    await db.delete(doc)
    await db.commit()