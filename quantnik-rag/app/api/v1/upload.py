...
async def upload_documents(
    ...
    project_name:     str             = Form(..., min_length=1, max_length=255, description="Project name — required gatekeeper"),
    ...
    async with AsyncSessionLocalNonCritical() as db_nc:
        for pending in pending_uploads:
            ...
            if await _is_exact_duplicate(db, db_nc, pending["filename"], pending["content_hash"], project_name):
...
async def _is_exact_duplicate(
    ...
    result_nc = await db_nc.execute(
        select(DocumentNonCritical).where(
            DocumentNonCritical.project_name  == project_name,
