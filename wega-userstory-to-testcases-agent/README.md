# WEGA User Story to Test Cases Agent
...
curl -X POST "http://localhost:8080/v1/generate-test-cases/bulk" \
  ...
  -d '{
    "userStories": [
      {"userStoryJiraId": "WEGA-123", "userStory": "As a user, I want to login"}
    ],
...
curl -X POST "http://localhost:8080/v1/generate-test-cases/bulk/qtest" \
  ...
  -d '{
    "userStories": [
      {"userStoryJiraId": "WEGA-124", "userStory": "As a user, I want to logout"}
    ],
...
curl -X POST "http://localhost:8080/v1/generate-test-cases/bulk/ado" \
  ...
  -d '{
    "userStories": [
      {"userStoryJiraId": "WEGA-125", "userStory": "As an admin, I want role-based access control"}
    ],
    "ScenarioTypes": ["Functional"]
  }'
```
...
curl -X POST "http://localhost:8080/v1/generate-test-cases/bulk/brownfield" \
  ...
  -d '{
    "userStories": [
      {"userStoryJiraId": "WEGA-126", "userStory": "As a user, I want MFA at sign-in"}
    ],
...
```bash
docker build -t wega-userstory-to-testcases-agent:latest .
```

Run container:

```bash
docker run --rm -p 8080:8080 --env-file .env wega-userstory-to-testcases-agent:latest
```
