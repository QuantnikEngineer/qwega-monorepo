# Deploy BRD-Agent-GCP to digital-rig-poc Project

Complete guide to deploy the BRD-Agent-GCP application to Google Cloud Compute Engine in the `digital-rig-poc` project.
...
tar -czf brd-agent-gcp.tar.gz brd_analyzer.py requirements.txt brd-summary.service
...
sudo cp brd_analyzer.py requirements.txt brd-summary.service /opt/brd-agent-gcp/
...
sudo cp brd-summary.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable brd-summary
sudo systemctl start brd-summary

# Check status
sudo systemctl status brd-summary
```
...
sudo nano /etc/systemd/system/brd-summary.service
...
# Restart service
sudo systemctl daemon-reload
sudo systemctl restart brd-summary
```
...
# View service logs
gcloud compute ssh $INSTANCE_NAME --zone=$ZONE \
    --command="sudo journalctl -u brd-summary -f"
...
```bash
gcloud compute ssh $INSTANCE_NAME --zone=$ZONE \
    --command="sudo systemctl restart brd-summary"
```
...
```bash
gcloud compute ssh $INSTANCE_NAME --zone=$ZONE \
    --command="sudo systemctl status brd-summary"
```
...
gcloud compute ssh $INSTANCE_NAME --zone=$ZONE --command="
    cd /tmp && \
    tar -xzf brd-agent-gcp.tar.gz && \
    sudo cp brd_analyzer.py requirements.txt /opt/brd-agent-gcp/ && \
    sudo systemctl restart brd-summary
"
```
...
```bash
gcloud compute ssh $INSTANCE_NAME --zone=$ZONE \
    --command="sudo journalctl -u brd-summary -n 100 --no-pager"
```
