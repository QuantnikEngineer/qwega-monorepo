# WEGA Code Review Agent

WEGA (WEbhook GitHub Agent) is an AI-powered code review service that automatically reviews pull requests using Google's Gemini AI. It integrates with GitHub and Jira to provide comprehensive, automated code reviews aligned with your project's requirements.

## About

WEGA Code Review Agent is a FastAPI-based microservice that:
...
2. **Configure environment**
   ...
   - `GITHUB_PRIVATE_KEY` - **Behavior depends on `ENVIRONMENT`**:
     - `development` -> path to the PEM file (e.g. `~/Downloads/wega.pem`)
...
WEGA uses **GitHub App installation tokens**, which are minted and refreshed
...
docker run -p 8001:8001 \
  ...
  -e GITHUB_PRIVATE_KEY=/secrets/wega.pem \
  -v $HOME/Downloads/wega.pem:/secrets/wega.pem:ro \
...
docker run -p 8001:8001 \
  ...
  -e GITHUB_APP_ID=123456 \
  -e GITHUB_INSTALLATION_ID=12345678 \
  -e GITHUB_PRIVATE_KEY="$(cat /path/to/wega.pem)" \
  -e GOOGLE_API_KEY=your_key \
  wega-cara
```
...
WEGA automatically reviews PRs when they're opened or updated.
...
## WEGA Orchestrator Integration

WEGA can work alongside the WEGA Orchestrator to provide a complete AI-driven development workflow. The orchestrator coordinates multiple AI agents that handle different aspects of the development process.

### Integration Points

1. **Webhook Coordination**: WEGA receives PR events from GitHub and processes reviews. Results can be fed back to the orchestrator for further actions.
...
GitHub PR Event
       ↓
WEGA Orchestrator (coordinates agents)
       ↓
  ┌────┴────┐
  ↓         ↓
WEGA    Other Agents
...
When running with the WEGA Orchestrator, ensure:
