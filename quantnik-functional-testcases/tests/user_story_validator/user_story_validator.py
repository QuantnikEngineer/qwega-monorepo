...
def test_08_wait_for_stories(shared_state):
    ...
    page = shared_state.page
    page.locator("text=/QUANTNIKAIDEMO-\\d+/").first.wait_for(state="visible", timeout=600000)
...
def test_09_select_and_update_first_story(shared_state):
    ...
    page = shared_state.page
    first_story = page.locator("text=/QUANTNIKAIDEMO-\\d+/").first
    first_story.click()
...
def test_10_select_and_update_second_story(shared_state):
    ...
    page = shared_state.page
    stories = page.locator("text=/QUANTNIKAIDEMO-\\d+/")
    if stories.count() > 1:
        stories.nth(1).click()
