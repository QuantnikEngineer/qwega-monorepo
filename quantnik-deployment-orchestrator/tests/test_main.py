...
def test_repository_context_endpoint_returns_harness_project_url(monkeypatch):
    ...
    monkeypatch.setattr(main_module.settings, "harness_project_identifier", "quantnik-build")
    client = TestClient(app)
    ...
    assert response.status_code == 200
    assert response.json() == {
        "repository_url": "https://app.harness.io/ng/account/acct-1/module/code/orgs/default/projects/quantnik-build"
    }
...
def test_repository_endpoint_accepts_authorized_identity_headers(monkeypatch):
    ...
    response = client.get(
        ...
        params={"platform": "github-actions"},
        headers={
            "X-QUANTNIK-Principal": "user@example.com",
            "X-QUANTNIK-Roles": "repo-reader,viewer",
        },
    )
