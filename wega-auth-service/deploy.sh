#!/bin/bash
# =============================================================================
# WEGA Auth Service - Deployment Script
# =============================================================================
# Usage:
#   ./deploy.sh                     # Deploy without profile
#   ./deploy.sh --profile dev       # Deploy with dev profile (dev-wega-auth-service)
...
SERVICE_BASE_NAME="wega-auth-service"
PROFILE=""
REGION="us-central1"
PROJECT=""
DRY_RUN=false
...
show_help() {
    echo "WEGA Auth Service - Deployment Script"
    echo ""
...
FRONTEND_SERVICE_NAME="${PROFILE_PREFIX}wega-frontend"
...
echo ""
echo "=========================================="
echo "  WEGA Auth Service — Deployment"
echo "=========================================="
...
echo "  Reg. Project:    ${REGISTRATION_DEFAULT_PROJECT_SLUG:-wega-sdlc-dev}"
...
# Build job env vars (migration only needs DATABASE_URL + WEGA_SECRET_KEY)
JOB_ENV="--set-env-vars APP_ENV=${APP_ENV}"

if [[ "$USE_SECRETS" == true ]]; then
    JOB_SECRET_ARGS="--set-secrets DATABASE_URL=${PROFILE_PREFIX}wega-auth-database-url:latest"
    JOB_DB_ARGS=""
else
    JOB_SECRET_ARGS=""
...
if [[ -n "${WEGA_SECRET_KEY:-}" && "$USE_SECRETS" == false ]]; then
    JOB_ENV="${JOB_ENV} --set-env-vars WEGA_SECRET_KEY=${WEGA_SECRET_KEY}"
fi
...
DEPLOY_CMD="gcloud run deploy ${SERVICE_NAME} \
    ...
    --set-env-vars REGISTRATION_DEFAULT_PROJECT_SLUG=${REGISTRATION_DEFAULT_PROJECT_SLUG:-wega-sdlc-dev} \
...
if [[ "$USE_SECRETS" == true ]]; then
    ...
    --update-secrets=/app/keys/private.pem=${PROFILE_PREFIX}wega-auth-jwt-private-key:latest \
    --update-secrets=/app/keys/public.pem=${PROFILE_PREFIX}wega-auth-jwt-public-key:latest \
    --update-secrets=DATABASE_URL=${PROFILE_PREFIX}wega-auth-database-url:latest \
    --update-secrets=INTERNAL_API_KEY=${PROFILE_PREFIX}wega-internal-api-key:latest \
    ...
    if [[ -z "${DATABASE_URL:-}" ]]; then
        ...
        log_error "Expected: postgresql+asyncpg://user:pass@host:5432/wega_auth"
        exit 1
    fi
    log_info "DATABASE_URL sourced (driver: $(echo "$DATABASE_URL" | cut -d: -f1-2))"

    # --- Read WEGA_SECRET_KEY (Fernet key for project secret encryption) ---
    if [[ -z "${WEGA_SECRET_KEY:-}" ]]; then
        if [[ -f .env ]]; then
            WEGA_SECRET_KEY=$(grep -E '^WEGA_SECRET_KEY=' .env | head -1 | cut -d'=' -f2-)
        fi
    fi
    ...
    if [[ -n "${WEGA_SECRET_KEY:-}" ]]; then
        DEPLOY_CMD="${DEPLOY_CMD} \
    --set-env-vars WEGA_SECRET_KEY=${WEGA_SECRET_KEY}"
        log_info "WEGA_SECRET_KEY sourced from .env"
    else
        log_warning "WEGA_SECRET_KEY not set — project secret encryption will use ephemeral key"
    fi
fi
