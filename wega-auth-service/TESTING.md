# WEGA Auth Service — Testing Guide (Phase 3)
...
|---------|------------|---------|
| Auth Service | `http://localhost:8090` | `python run.py` (from `wega-auth-service/`) |
| API Gateway | `http://localhost:8080` | `python run.py` (from `wega-api-gateway/`) |
| Frontend | `http://localhost:3000` | `npm run dev` (from `wega-frontend/`) |

Ensure migrations have been applied:
```bash
cd wega-auth-service
...
```bash
cd wega-auth-service
python scripts/create_superadmin.py --email admin@wipro.com --name "Admin" --password "WegaAdmin1!@#"
```
...
WEGA defines **6 roles** across three scope tiers:
...
|-------|-------------|------|-------|----------|
| `aniket.ashtikar@wipro.com` | Aniket Ashtikar | superadmin | platform | `WegaAdmin1!@#` |
...
curl -X POST http://localhost:8090/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"aniket.ashtikar@wipro.com","password":"WegaAdmin1!@#"}' \
  -c cookies.txt
```
