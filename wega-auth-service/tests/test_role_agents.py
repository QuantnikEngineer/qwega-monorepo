...
GET /api/roles/agents, and that the WEGA Orchestrator is excluded from
...
class TestRoleAgentMapping:
    ...
    async def test_orchestrator_excluded(
        ...
        """D-12: WEGA Orchestrator hidden from all users."""
        response = await test_client.get(
            "/api/roles/agents",
            headers=superadmin_headers,
        )
        agents = response.json()["agents"]
        assert "Wega-Orchestrator" not in agents
        assert "wega-orchestrator" not in agents
