...
## Harness CI/CD Implementation Plan for QUANTNIK Pipelines
...
|-------|-------------------|---------|
| GCP | `quantnik-{env}-{secret-name}` | `quantnik-dev-db-password` |
| Azure | Vault: `quantnik-kv-{env}`, Secret: `{name}` | `quantnik-kv-dev/db-password` |
| AWS | `quantnik/{env}/{secret-name}` | `quantnik/dev/db-password` |
...
./scripts/secrets-setup/setup-gcp-secrets.sh quantnik-gcp-project

# Azure - Creates Key Vaults and secrets
./scripts/secrets-setup/setup-azure-secrets.sh quantnik-azure-rg eastus
...
|--------|---------|------------------|
| `setup-gcp-secrets.sh` | `quantnik-{env}-{secret}` in GCP Secret Manager | Cloud Run service account |
| `setup-azure-secrets.sh` | `quantnik-kv-{env}` vaults + secrets | ACA managed identity |
| `setup-aws-secrets.sh` | `quantnik/{env}/{secret}` in AWS Secrets Manager | ECS task role |
    ...
    spec:
      command: |
        ...
        SECRETS=$(yq -r '.secrets[].envVar' "$REPO_PATH/config/secrets.yaml" | while read secret; do
          echo "${secret}=quantnik-${ENV}-$(echo $secret | tr '[:upper:]' '[:lower:]' | tr '_' '-'):latest"
        done | paste -sd ',' -)
...
- step:
    ...
    spec:
      command: |
        ...
        SECRETS_JSON=$(yq -r ".secrets | map({name: .envVar, valueFrom: \"arn:aws:secretsmanager:us-east-1:${AWS_ACCOUNT}:secret:quantnik/${ENV}/\" + (.envVar | ascii_downcase | gsub(\"_\"; \"-\"))})" \
...
- [ ] Add config/ folder to pilot service (quantnik-brd)
...
- [ ] Create config/ folder for all 23 QUANTNIK 3.0 services
- [ ] Deploy to Dev
- [ ] Deploy to QA
- [ ] Deploy to Staging
...
*QUANTNIK CI/CD - Environment & Secrets Management Plan v3.0*
