#!/bin/bash
# AWS ECS Cluster Setup Script for QUANTNIK 3.0
...
echo "==========================================="
echo "AWS ECS Cluster Setup for QUANTNIK 3.0"
...
for ENV in "${ENVIRONMENTS[@]}"; do
  ...
  if [ "$EXISTING" = "ACTIVE" ]; then
    ...
    aws ecs create-cluster \
      ...
      --region "$AWS_REGION" \
      --tags key=Environment,value="$ENV" key=Project,value=QUANTNIK key=ManagedBy,value=Harness
