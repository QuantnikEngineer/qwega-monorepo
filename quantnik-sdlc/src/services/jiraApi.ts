 ...
 *   VITE_ATLASSIAN_BASE_URL – e.g. https://quantnikbuildiq.atlassian.net (for browse links)
...
export interface JiraIssue {
  ...
  updated: string;
  children?: JiraIssue[];
  /** Full URL to the issue in Jira, e.g. https://quantnikbuildiq.atlassian.net/browse/QUANTNIKAIDEMO-1 */
  url: string;
}
...
const _FALLBACK_BASE_URL: string = _env.VITE_ATLASSIAN_BASE_URL || 'https://quantnikbuildiq.atlassian.net';
const _FALLBACK_PROJECT_KEY: string = _env.VITE_JIRA_PROJECT_KEY || 'QUANTNIKAIDEMO';
...
export async function searchJiraIssues(
  ...
  const isFullKey = /^[A-Z][A-Z0-9]+-\d+$/i.test(trimmed);     // e.g. QUANTNIKAIDEMO-2652
 ...
 * Primary approach: search by label matching the story key (e.g. labels = "QUANTNIKAIDEMO-701").
