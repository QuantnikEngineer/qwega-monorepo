...
# Cloud Run injects K_SERVICE (e.g. "dev-quantnik-sdlc").
...
if [ -z "$GATEWAY_UPSTREAM" ]; then
    # K_SERVICE is set by Cloud Run (e.g. "dev-quantnik-sdlc", "qa-quantnik-sdlc")
    if [ -n "$K_SERVICE" ]; then
        ENV_PREFIX=$(echo "$K_SERVICE" | cut -d'-' -f1)
        PROJECT_NUMBER="${GCP_PROJECT_NUMBER:-204952354085}"
        REGION="${GCP_REGION:-us-central1}"
        GATEWAY_UPSTREAM="https://${ENV_PREFIX}-quantnik-api-gateway-${PROJECT_NUMBER}.${REGION}.run.app"
