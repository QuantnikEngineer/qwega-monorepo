# WEGA Anomaly Agent - Troubleshooting Guide
...
docker build -t wega-anomaly:latest . 2>&1 | tee build.log
...
export ECR_URI="145748108830.dkr.ecr.ap-south-1.amazonaws.com/wega-wega-anomaly"

# Rebuild
docker build -t wega-wega-anomaly:${VERSION} .

# Tag for ECR
docker tag wega-wega-anomaly:${VERSION} ${ECR_URI}:${VERSION}
docker tag wega-wega-anomaly:${VERSION} ${ECR_URI}:latest
...
sed -i "s|wega-wega-anomaly:1.0.0|wega-wega-anomaly:${VERSION}|g" deploy/kubernetes/deployment.yaml
...
**Error:**
```
wega-anomaly-agent-xxx   0/1     ErrImagePull      0          31s
wega-anomaly-agent-xxx   0/1     ImagePullBackOff  0          59s
```
...
# If it still shows "wipro/wega-anomaly-agent:latest", the sed didn't work
```
...
#   image: 145748108830.dkr.ecr.ap-south-1.amazonaws.com/wega-wega-anomaly:1.0.1

# Save and exit (Ctrl+X, Y, Enter)

# Delete and re-apply
kubectl delete deployment wega-anomaly-agent -n wega-system
kubectl apply -f deploy/kubernetes/deployment.yaml
...
kubectl describe pod -l app=wega-anomaly-agent -n wega-system | grep -A 10 "Events:"
```
...
**Error:**
```
wega-anomaly-agent-xxx   0/1     CrashLoopBackOff   5          5m
```
...
kubectl logs -l app=wega-anomaly-agent -n wega-system --previous
...
kubectl get configmap wega-anomaly-config -n wega-system
...
**Error:**
```
Error from server (NotFound): namespaces "wega-system" not found
```
...
- Different image name (e.g., `wega-anomaly-agent` vs `wega-wega-anomaly`)
- Extra spaces
- Different tag
...
aws ecr create-repository \
    --repository-name wega-wega-anomaly \
    --region ap-south-1
...
```bash
kubectl delete ingress wega-anomaly-agent -n wega-system
...
2. Verify pods are running: `kubectl get pods -n wega-system`
3. Test health endpoint from within pod:
   ```bash
   kubectl exec -it $(kubectl get pod -l app=wega-anomaly-agent -n wega-system -o jsonpath='{.items[0].metadata.name}') -n wega-system -- curl -s http://localhost:8080/health
   ```
...
# ===== Kubernetes =====
kubectl get pods -n wega-system              # List pods
kubectl logs -l app=wega-anomaly-agent -n wega-system   # Pod logs
kubectl describe pod <pod-name> -n wega-system          # Pod details
kubectl get events -n wega-system --sort-by='.lastTimestamp'  # Recent events
