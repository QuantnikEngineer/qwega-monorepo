# WEGA Anomaly Agent - Context Document
...
- **End Users**: Interact via wega-sdlc frontend (Dashboard + Chatbot)
...
│   └── WEGA_SDLC_INTEGRATION.md    # Frontend integration guide
│
├── src/                            # Source code
...
### 8.3 wega-sdlc Frontend → Anomaly Agent
...
```bash
cd ~/wega-projects/anomaly-agent
...
docker run -d -p 8080:8080 --name test wega-anomaly-agent:latest
docker logs test
...
docker tag wega-anomaly-agent:latest <ecr-uri>:latest
...
kubectl logs -l app=wega-anomaly-agent -n wega-system
kubectl port-forward svc/wega-anomaly-agent -n wega-system 8080:80
kubectl rollout restart deployment/wega-anomaly-agent -n wega-system
```
...
| **Public URL** | ✅ Active | `http://k8s-wegasyst-wegaanom-0573c24f8a-975249009.ap-south-1.elb.amazonaws.com` |
...
| **ECR Image** | ✅ Pushed | `145748108830.dkr.ecr.ap-south-1.amazonaws.com/wega-wega-anomaly:1.0.1` |
| **Harness Pipeline** | ✅ Created | `AnomalyAgent-CICD` in `WEGA_Build_AI` project |

### Kubernetes Resources

```
Namespace: wega-system
Deployment: wega-anomaly-agent (2 replicas)
Service: wega-anomaly-agent (ClusterIP, port 80 → 8080)
Ingress: wega-anomaly-agent (ALB, internet-facing)
ConfigMap: wega-anomaly-config
Secret: wega-anomaly-secrets
```
...
| 3 | Frontend integration (wega-sdlc) | 2-4 hours |
...
### Current wega-sdlc UI Structure
...
- Selected agent shows a chatbot interface with "Welcome! I am wega agent. How can I help you?"
...
```bash
# Add to wega-sdlc/.env
VITE_ANOMALY_AGENT_URL=http://k8s-wegasyst-wegaanom-0573c24f8a-975249009.ap-south-1.elb.amazonaws.com
```
...
```bash
# Health check
curl http://k8s-wegasyst-wegaanom-0573c24f8a-975249009.ap-south-1.elb.amazonaws.com/health

# Swagger UI
http://k8s-wegasyst-wegaanom-0573c24f8a-975249009.ap-south-1.elb.amazonaws.com/docs

# Chat with AI
curl -X POST http://k8s-wegasyst-wegaanom-0573c24f8a-975249009.ap-south-1.elb.amazonaws.com/api/v1/chat \
...
| Frontend Integration | `docs/WEGA_SDLC_INTEGRATION.md` | wega-sdlc changes |
...
| Frontend Team | wega-sdlc integration |
