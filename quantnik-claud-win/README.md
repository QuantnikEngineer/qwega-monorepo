...
- **Local development** (this page): Linux / macOS, Node 20+.
- **Windows Server with IIS**: see [`docs/WINDOWS.md`](docs/WINDOWS.md) for the
  IIS reverse-proxy setup, Windows service install (via `node-windows`), and
  PowerShell deploy script.
- **Container / Cloud Run**: the `Dockerfile` and `docker-compose.yml` at the
  repo root still work; user-level skills are baked in via
  `scripts/snapshot-claude-user.sh` and the `.claude-user/` directory.

## Prerequisites (local dev)

- Node 20+
- `claude` CLI authenticated on your machine, **or** an `ANTHROPIC_API_KEY`
  available to the backend process, **or** a `CLAUDE_CODE_OAUTH_TOKEN` from
  `claude setup-token`. The Claude Agent SDK uses whichever it finds.
