...
export default function App() {
  ...
  const refresh = async () => {
    if (!authToken.get()) {
      // Don't clear the project list on logout — the AuthGate overlay hides
      // the UI anyway. Clearing here makes projects appear deleted to the user;
      // they come back on re-login but the flash of empty state is confusing.
      return;
    }
    let list;
    try {
      ...
      // token clear. Preserve the current list so projects do not appear to
      // vanish transiently.
      return;
    }
    setProjects(list);
