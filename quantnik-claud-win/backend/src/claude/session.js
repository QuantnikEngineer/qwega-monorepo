...
function buildQueryOptions(project, repoPaths, requestPermission, resumeId, llmModel, abortController) {
  return {
    ...
    skills: 'all',
    mcpServers: (() => {
      const envServers = getMcpServersFromEnv();
      try {
        const settingsFile = path.join(resolveProjectPath(project), '.claude', 'settings.json');
        if (fs.existsSync(settingsFile)) {
          const local = JSON.parse(fs.readFileSync(settingsFile, 'utf8'));
          if (local.mcpServers && typeof local.mcpServers === 'object') {
            return { ...envServers, ...local.mcpServers };
          }
        }
      } catch {}
      return envServers;
