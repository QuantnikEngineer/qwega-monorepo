...
export function getMcpServersFromEnv() {
  ...
  if ((process.env.MCP_ATLASSIAN_TOKEN || process.env.MCP_ATLASSIAN_EMAIL) && !atlassianSite) {
    console.warn('[config] MCP_ATLASSIAN_TOKEN/EMAIL are set but MCP_ATLASSIAN_URL / MCP_ATLASSIAN_SITE_NAME is missing — Atlassian (Jira/Confluence) MCP will be skipped. Set MCP_ATLASSIAN_URL=https://<your-domain>.atlassian.net in .env to enable it.');
  }
