...
def test_harness_repository_lookup_returns_project_repositories(monkeypatch):
    ...
    monkeypatch.setattr(repository_lookup_module.settings, 'harness_project_identifier', 'WEGA_Build_AI')
    ...
    async def fake_fetch_json(url, headers, provider):
        ...
        assert url == (
            ...
            'accountIdentifier=2KolbecvR0aAcgQ5uXBObA&orgIdentifier=default&projectIdentifier=WEGA_Build_AI&page=1&limit=100'
        )
        assert headers == {'Accept': 'application/json', 'x-api-key': 'test-token'}
        return [
            {
                'identifier': 'bdd-agent',
                'git_url': 'https://git.harness.io/2KolbecvR0aAcgQ5uXBObA/default/WEGA_Build_AI/bdd-agent.git',
            },
            {
                'identifier': 'wega-api',
                'git_url': 'https://git.harness.io/2KolbecvR0aAcgQ5uXBObA/default/WEGA_Build_AI/wega-api.git',
            },
        ]
    ...
    assert repositories == [
        RepositoryOption(
            id='bdd-agent',
            label='bdd-agent',
            url='https://git.harness.io/2KolbecvR0aAcgQ5uXBObA/default/WEGA_Build_AI/bdd-agent.git',
        ),
        RepositoryOption(
            id='wega-api',
            label='wega-api',
            url='https://git.harness.io/2KolbecvR0aAcgQ5uXBObA/default/WEGA_Build_AI/wega-api.git',
        ),
    ]
...
def test_harness_repository_lookup_paginates_beyond_default_first_page(monkeypatch):
    ...
    async def fake_fetch_json(url, headers, provider):
            ...
            return [
                {
                    ...
                    'git_url': f'https://git.harness.io/2KolbecvR0aAcgQ5uXBObA/default/WEGA_Build_AI/repo-{index:03d}.git',
                }
        ...
        if url.endswith('&page=2&limit=100'):
            return [
                {
                    'identifier': 'wega-userstory-estimator',
                    'git_url': 'https://git.harness.io/2KolbecvR0aAcgQ5uXBObA/default/WEGA_Build_AI/wega-userstory-estimator.git',
                }
            ]
    ...
    assert observed_urls == [
        'https://app.harness.io/gateway/code/api/v1/repos?accountIdentifier=2KolbecvR0aAcgQ5uXBObA&orgIdentifier=default&projectIdentifier=WEGA_Build_AI&page=1&limit=100',
        'https://app.harness.io/gateway/code/api/v1/repos?accountIdentifier=2KolbecvR0aAcgQ5uXBObA&orgIdentifier=default&projectIdentifier=WEGA_Build_AI&page=2&limit=100',
    ]
    assert any(repository.label == 'wega-userstory-estimator' for repository in repositories)
...
def test_harness_branch_lookup_uses_selected_repository_git_url(monkeypatch):
    ...
    branches = asyncio.run(
        client.list_branches(
            'harness',
            'https://git.harness.io/2KolbecvR0aAcgQ5uXBObA/default/WEGA_Build_AI/bdd-agent.git',
        )
    )
...
def test_harness_branch_lookup_requires_repository_selection(monkeypatch):
    ...
    with pytest.raises(RepositoryLookupError, match='Select a repository from the loaded list first'):
        asyncio.run(
            client.list_branches(
                'harness',
                'https://app.harness.io/ng/account/2KolbecvR0aAcgQ5uXBObA/module/code/orgs/default/projects/WEGA_Build_AI',
            )
        )
