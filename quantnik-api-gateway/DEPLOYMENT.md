# QUANTNIK API Gateway — Deployment Guide
...
| `JWT_ISSUER` | | `quantnik-auth` | Must match auth service |
| `JWT_AUDIENCE` | | `quantnik-api` | Must match auth service |
| `INTERNAL_API_KEY` | ✅ | `quantnik-internal-dev-key` | Must match auth service |
...
```
AUTH_SERVICE_URL = https://{profile}-quantnik-auth-service-{PROJECT_NUMBER}.{REGION}.run.app
ORCHESTRATOR_URL = https://{profile}-quantnik-sdlc-orchestrator-{PROJECT_NUMBER}.{REGION}.run.app
```
...
Requires `quantnik-auth-service` running on port 8090.
...
5. **Deployment order matters**: `quantnik-auth-service` → `quantnik-api-gateway` → `quantnik-frontend`. Each depends on the previous. The gateway needs the auth-service URL at deploy time.
...
| Default | `quantnik-internal-dev-key` | `quantnik-internal-dev-key` |
