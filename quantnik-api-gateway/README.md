# Quantnik API Gateway

Central API gateway for the QUANTNIK platform. Sits between the frontend and all backend services, handling JWT validation, CORS, rate limiting, capability-based authorization, audit logging, and reverse-proxying to upstream services.
...
| quantnik-auth-service | Running | Must be available at `:8090` for JWT/JWKS |

> **Important:** Start `quantnik-auth-service` BEFORE starting the gateway. The gateway fetches JWKS from auth-service for JWT validation.
...
If this fails, ensure `quantnik-auth-service` is running on port 8090.
...
| `JWT_ISSUER` | Expected JWT issuer claim | `quantnik-auth` |
| `JWT_AUDIENCE` | Expected JWT audience claim | `quantnik-api` |
...
```
https://{prefix}quantnik-{service}-{project_number}.{region}.run.app
```
...
```bash
# Standard build
docker build -t quantnik-api-gateway .

# Override base image registry (used by CI/CD pipelines)
docker build --build-arg BASE_REGISTRY=your-registry.example.com -t quantnik-api-gateway .

docker run -p 8080:8080 --env-file .env quantnik-api-gateway
```
...
- **quantnik-auth-service** already deployed (gateway needs `AUTH_SERVICE_URL` pointing to it)
...
- Deploys to Cloud Run with profile-based naming (`dev-quantnik-api-gateway`, `prod-quantnik-api-gateway`)
...
|----------|----------|---------|
| `AUTH_SERVICE_URL` | Yes | `https://dev-quantnik-auth-service-XXXXX.us-central1.run.app` |
| `ORCHESTRATOR_URL` | Yes | `https://dev-quantnik-orchestrator-XXXXX.us-central1.run.app` |
| `PLANNING_ORCHESTRATOR_URL` | Yes | `https://dev-quantnik-planning-orchestrator-XXXXX.us-central1.run.app` |
| `INTERNAL_API_KEY` | Yes | Shared secret with auth-service |
| `JWT_ISSUER` | No | `quantnik-auth` (default — must match auth-service) |
| `JWT_AUDIENCE` | No | `quantnik-api` (default — must match auth-service) |
| `CORS_ORIGINS` | Yes | `https://dev-quantnik-frontend-XXXXX.us-central1.run.app` |
...
- `JWT_ISSUER` = `quantnik-auth` (both services)
- `JWT_AUDIENCE` = `quantnik-api` (both services)
...
```bash
# Health check
curl https://dev-quantnik-api-gateway-XXXXX.us-central1.run.app/health
# Expected: {"status": "ok"}

# JWKS endpoint (verifies gateway → auth-service connectivity)
curl https://dev-quantnik-api-gateway-XXXXX.us-central1.run.app/auth/jwks
# Expected: {"keys": [{"kty": "RSA", ...}]}

# Login (verifies full auth flow)
curl -X POST https://dev-quantnik-api-gateway-XXXXX.us-central1.run.app/auth/login \
...
```
1. quantnik-auth-service   (must be running for JWKS/auth)
2. quantnik-api-gateway    ← YOU ARE HERE
3. quantnik-frontend       (needs GATEWAY_UPSTREAM pointing here)
```
