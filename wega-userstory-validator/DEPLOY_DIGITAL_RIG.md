# Deploy Wega-userstory-validator to digital-rig-poc Project

Complete guide to deploy the Wega-Userstory-Validator application to Google Cloud Compute Engine in the `digital-rig-poc` project.
...
export INSTANCE_NAME="wega-validator-instance"
...
gcloud compute firewall-rules create allow-wega-validator-8080 \
    --project=$PROJECT_ID \
    --allow=tcp:8080 \
    --description="Allow Wega Userstory Validator API on port 8080" \
    --direction=INGRESS \
    --target-tags=wega-validator
```
...
tar -czf Wega-userstory-validator.tar.gz server.py requirements.txt Wega-userstory-validator.service

# Copy to instance
gcloud compute scp Wega-userstory-validator.tar.gz $INSTANCE_NAME:/tmp/ --zone=$ZONE
```
...
# Extract files
cd /tmp
tar -xzf Wega-userstory-validator.tar.gz

# Create application directory
sudo mkdir -p /opt/Wega-userstory-validator
sudo cp server.py requirements.txt Wega-userstory-validator.service /opt/Wega-userstory-validator/
cd /opt/Wega-userstory-validator

# Create virtual environment
sudo python3 -m venv venv

# Install dependencies
sudo /opt/Wega-userstory-validator/venv/bin/pip install --upgrade pip
sudo /opt/Wega-userstory-validator/venv/bin/pip install -r requirements.txt

# Set up systemd service
sudo cp Wega-userstory-validator.service /etc/systemd/system/
