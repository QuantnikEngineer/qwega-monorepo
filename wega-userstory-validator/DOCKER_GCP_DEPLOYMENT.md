...
This guide covers deploying the Wega Userstory Validator using Docker on Google Cloud Platform. It supports multiple deployment options: **Cloud Run** (recommended), **GKE**, and **Compute Engine**.
...
gcloud builds submit --tag ${REGION}-docker.pkg.dev/${PROJECT_ID}/wega-userstory-validator-repo/wega-userstory-validator

# 3. Deploy to Cloud Run
gcloud run deploy wega-userstory-validator \
    --image ${REGION}-docker.pkg.dev/${PROJECT_ID}/wega-userstory-validator-repo/wega-userstory-validator \
    --region ${REGION} \
    --platform managed \
...
```bash
# Build the image
docker build -t wega-userstory-validator:latest .

# Verify the image
docker images | grep wega-userstory-validator
```
...
docker run -p 8080:8080 \
    ...
    wega-userstory-validator:latest
...
export PROJECT_ID="your-project-id"
export REGION="us-central1"
export REPO_NAME="wega-userstory-validator-repo"

# Create repository
gcloud artifacts repositories create ${REPO_NAME} \
    --repository-format=docker \
    --location=${REGION} \
    --description="Wega Userstory Validator Docker images"
...
```bash
export IMAGE_URI="${REGION}-docker.pkg.dev/${PROJECT_ID}/${REPO_NAME}/wega-userstory-validator"
...
gcloud iam service-accounts create wega-validator-sa \
    --display-name="Wega Userstory Validator Service Account"

# Grant Vertex AI access
gcloud projects add-iam-policy-binding ${PROJECT_ID} \
    --member="serviceAccount:wega-validator-sa@${PROJECT_ID}.iam.gserviceaccount.com" \
...
gcloud run deploy wega-userstory-validator \
    ...
    --region ${REGION} \
    --platform managed \
    --service-account wega-validator-sa@${PROJECT_ID}.iam.gserviceaccount.com \
    --memory 1Gi \
    --cpu 1 \
    --timeout 300 \
    --concurrency 80 \
...
# Get the service URL
gcloud run services describe wega-userstory-validator --region ${REGION} --format='value(status.url)'
```
...
gcloud run services add-iam-policy-binding wega-userstory-validator \
    --region ${REGION} \
...
```bash
export CLUSTER_NAME="wega-validator-cluster"
...
kind: Deployment
metadata:
  name: wega-userstory-validator
  labels:
    app: wega-userstory-validator
spec:
  replicas: 2
  selector:
    matchLabels:
  ...
  template:
    metadata:
      labels:
        app: wega-userstory-validator
    spec:
      serviceAccountName: wega-validator-ksa
      containers:
        - name: wega-userstory-validator
          image: us-central1-docker.pkg.dev/PROJECT_ID/wega-userstory-validator-repo/wega-userstory-validator:latest
          ports:
            - containerPort: 8080
          env:
            - name: PORT
...
```bash
gcloud compute instances create-with-container wega-validator-vm \
    --zone ${ZONE} \
    --machine-type e2-medium \
...
gcloud compute firewall-rules create allow-wega-validator \
    --allow tcp:8080 \
...
gcloud builds triggers create github \
    --name="wega-validator-deploy" \
    --repo-name="Wega-userstory-validator" \
    --repo-owner="your-org" \
    --branch-pattern="^main$" \
    --build-config="cloudbuild.yaml" \
    --substitutions="_SERVICE_NAME=wega-userstory-validator,_REGION=us-central1"
```
...
```bash
# View service logs
gcloud run services logs read wega-userstory-validator --region ${REGION}

# Stream logs
gcloud run services logs tail wega-userstory-validator --region ${REGION}
```
...
```bash
# Create uptime check
gcloud monitoring uptime-checks create http wega-validator-uptime \
    --display-name="Wega Userstory Validator Health" \
...
gcloud run services logs read wega-userstory-validator --region ${REGION} --limit=50
```
...
gcloud projects get-iam-policy ${PROJECT_ID} \
    ...
    --filter="bindings.members:wega-validator-sa"
```

**4. Timeout Errors**
```bash
# Increase timeout (max 3600s for Cloud Run)
gcloud run services update wega-userstory-validator \
    --region ${REGION} \
    --timeout 600
```
